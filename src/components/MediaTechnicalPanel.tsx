...
export type MediaAnalysisContentProps = {
    ...
    <section className={cn(compactPanelClass, 'relative overflow-visible')}>
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          type="button"
          className={cn(
            'btn-technical shrink-0 gap-1.5 px-2 py-1 text-[0.58rem] tracking-[0.06em]',
...
export function MediaAnalysisContent({
  mediaSummary,
  selectedFile,
  baseAnalysis,
  derivedAnalysis,
  detailed = false,
}: MediaAnalysisContentProps) {
  if (!selectedFile) {
    return (
      <div className="grid gap-3">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-technical text-ozone-accent">Media Analysis</h3>
          <span className={cn(getPillClass('neutral'), 'px-1.5 py-0.5 text-[0.42rem] opacity-80')}>
            Waiting for media
          </span>
        </div>

        <div className="rounded-sm border border-ozone-border-bright bg-black/35 p-3">
          <p className="text-[0.7rem] leading-relaxed text-ozone-text-muted">
            Load a video to view the technical analysis, metadata, codecs and loudness diagnostics.
          </p>

          <div className="mt-3 grid grid-cols-2 gap-2.5">
            {['Size', 'Duration', 'Format', 'Audio Codec'].map((label) => (
              <div key={label} className="rounded-sm border border-ozone-border bg-black/30 px-2.5 py-2">
                <span className="text-[0.48rem] text-technical text-ozone-text-muted/80">
                  {label}
                </span>
                <div className="mt-1 h-3.5 w-16 rounded-full bg-white/8" />
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="grid gap-3">
      {!detailed ? (
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <h3 className="text-technical text-ozone-accent">Media Analysis</h3>
            {derivedAnalysis ? (
              <span className={cn(getPillClass(derivedAnalysis.audioState), 'px-1.5 py-0.5 text-[0.4rem]')}>
                {derivedAnalysis.audioStateLabel}
              </span>
            ) : null}
          </div>
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 rounded-sm border border-ozone-border-bright bg-black/35 p-3">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 rounded-sm border border-ozone-border bg-black/30 p-3">
        {metrics.length > 0 ? (
          metrics.map((metric) => (
