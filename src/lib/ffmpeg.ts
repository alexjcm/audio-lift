...
import { parseProbeLogs } from './ffmpeg-parser'
...
function buildExportArgs(
  ...
  const limitLinear = mapDbToLinearGain(options.targetTruePeakDbtp)
  const limiterChain = [
    ...
    `alimiter=limit=${limitLinear.toFixed(6)}:level=false`,
  ...
  const filterComplex = [
    ...
    `[vb]lowpass=f=${options.virtualBassCutoffHz}${monoFold},asoftclip=type=tanh:param=${options.virtualBassDrive}:oversample=4,highpass=f=${harmonicHighpassHz},lowpass=f=${VIRTUAL_BASS_HARMONIC_LP_HZ},volume=${mixGain.toFixed(4)}${harmonicPan}[vbfx]`,
